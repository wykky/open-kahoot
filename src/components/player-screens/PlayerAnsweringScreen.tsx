    ...
    <div className="w-full flex flex-col justify-center sm:bg-white sm:rounded-lg sm:p-8 sm:border sm:border-gray-300 sm:shadow-[0px_20px_30px_-10px_rgba(0,_0,_0,_0.1)]">
      {showQuestion ? (
        <>
          ...
          <h2 className="text-lg sm:text-2xl font-bold text-black text-center mb-6 leading-snug px-2">
            {question?.question}
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 flex-1">
            {(question?.options ?? ['', '', '', '']).map((optionText, index) => (
              <button
                ...
                className={`min-h-20 sm:min-h-24 rounded-xl font-bold text-white transition-all transform hover:scale-105 border-4 ${getChoiceColor(index)} px-4 py-3 flex items-center gap-3 text-left`}
              >
                <span className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/30 flex items-center justify-center text-xl sm:text-2xl">
                  {['A', 'B', 'C', 'D'][index]}
                </span>
